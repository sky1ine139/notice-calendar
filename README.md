# 通知日程助手（NoticeCalendar）

一个**个人自用**的安卓小工具：把微信群 / QQ群里的通知消息粘贴进来，自动用大模型提取**事件标题、时间、地点、备注**，人工确认后**一键写入系统日历并设置提醒**。所有解析过的通知自动存入历史记录，可回看、再编辑、重新写入日历。

无广告、无登录注册、无多余功能。

**最新版本 v1.5.3** — [点击下载 APK](https://github.com/sky1ine139/notice-calendar/releases/latest)（约 25MB，正式签名，可直接覆盖升级）

### v1.5.3 修复

- 🐛 **修复"填了 API 地址却报 HTML/429"的问题**：很多人会把服务商的**官网/控制台地址**当成接口地址填（如 `https://platform.deepseek.com/`），那个地址返回的是网页而不是接口数据，报错信息里会混进一整段 `<!DOCTYPE html>` 源码
  - 现在**保存/测试连接时自动规范化地址**：控制台/官网地址自动换成真正的 API 域名，漏写 `/v1` 自动补上，多写 `/chat/completions` 自动去掉，只写域名自动补 `https://`
  - 打开设置页时会**回显修正后的地址**并提示"已自动修正接口地址/模型名，请点「保存」"
- 🐛 **模型名自动纠正**：只填服务商名（如 `deepseek`）时自动补成可用模型名（`deepseek-chat`）；`kimi`→`moonshot-v1-8k`、`qwen`→`qwen-plus`、`glm`→`glm-4-flash`
- 🐛 **报错信息说人话**：不再把 HTML 源码糊在屏幕上，改为明确提示——你填的地址是官网/控制台、请改成 API 地址并给出示例；常见状态码也有了中文解释（401 Key 无效 / 402 余额不足 / 404 地址或模型名不对 / 429 请求过频）
- 地址规范化与报错路径均已离线验证（21 个地址/模型用例 + 真实 OkHttp 打本地假服务器 4 项断言全部通过）

### v1.5.2 修复

- 🐛 **修复"志愿"类通知完全提取不出标题**：事件词表里原先没有"志愿者/志愿/服务"，导致「跟车志愿者统一乘坐7点大巴去机场」这种通知直接退回"取原文第一行"，标题变成长长的残句（`各位同学 的乘车安排有 变化所有的跟车志愿者(王鑫 陈仕鹏…`）
- ✨ **新增"志愿 + 交通"的组合标题**：通知里同时出现志愿活动与乘车/大巴/包车/接送时，标题直接给 **`乘车做志愿`**（时间、地点仍各自正确提取；地点在"学生活动中心"就填地点，不塞进标题）
- 🐛 **修复 `志愿者面试` 被截成 `志愿者`**：关键词后可合并的事件词（面试/招募/培训/考试…）现在会一起取出
- 现在这类通知的解析结果：标题 `乘车做志愿`、时间 `2026-09-16 07:00`、地点 `学生活动中心`
- 同样仅影响**不填 API Key 时的本地解析**

### v1.5.1 修复

- 🐛 **修复本地解析把标题提取错的严重问题**：像「笔试（CET）于2026年12月12日举行，开考科目为英语四级和六级」这类通知，标题曾被解析成 `笔试CET于 举行开考科目为英语四级和六级具` 这种残句；现在会正确提取成 **英语四级和六级笔试**
- 🐛 **修复标题尾巴粘连**：清理标题时中文逗号被当成空格删除，导致「…四级和六级，具体场次…」被粘成"级具"；现已改为按标点切分
- 🐛 **修复科目/等级信息被丢弃**：新增"科目 + 考试类型"识别（`英语四六级考试`、`计算机二级考试` 等）
- 🐛 **修复"通知/公告"被并进标题**：`班会通知` → `班会`，不再把通告词算作事件名
- 该修复仅影响**不填 API Key 时的本地解析**；配置了大模型 API 的用户解析路径不变

### v1.5 更新

- 🖼️ **图片通知识别（OCR）**：通知是一张截图/照片也不怕 —— 点「识别图片通知」从相册选图，自动把图里的文字提取到输入框，核对后照常「解析事件」
- 🔒 **本地离线识别**：文字识别在手机本地完成，**图片不上传任何服务器**；不填 API Key 也能用（首次使用需联网下载约 20MB 中文识别模型，之后离线可用）
- ✂️ **断行自动合并**：OCR 把一句通知拆成两行时会自动接回，减少解析误差
- 无需新增任何权限（Android 13+ 走系统相册选择器，旧系统走文档选择器）

### v1.4 更新

- 🎨 **深色模式修复**：输入框文字、占位符、打赏卡片等深色模式下不再看不清
- 🧠 **本地解析增强**：优先提取核心事件名（"志愿者面试"）；识别"面试地点在10110""候场地点在10113B"等房间号地点并完整保留
- ⏰ **无时间通知提示**：通知里没提时间时不再默默按"今天全天"，编辑页橙色高亮并提示补时间

### v1.3 新功能

- 🔄 **自动更新检查**：App启动时静默检查GitHub最新Release，有新版本自动弹出提示，显示更新日志和下载按钮
- 📲 **手动检查更新**：设置页新增「检查更新 / 获取最新版本」按钮，手动触发检查
- ℹ️ **设置页关于区域**：新增版本号、开源协议信息，方便用户查看当前版本

### v1.2 新功能

- 📅 **日程自动更新**：识别"延期/改期"类通知，自动匹配历史日程，保留原标题，删除旧日程后写入新时间
- 🔢 **中文数字解析**：支持"九月十日""八点半""下午三点一刻"等中文表达，不填API Key也能准确解析
- ✂️ **标题自动清理**：自动去掉日期时间连接词，只保留核心事件名，日历上更醒目
- 🎨 **5套主题换肤**：紫/蓝/绿/橙/深色，设置页切换即时生效
- ☕ **打赏支持**：首页底部"请作者喝瓶水"卡片，微信/支付宝收款码

---

## 一、功能与流程

1. 复制群通知 → 打开APP → 点「从剪贴板粘贴」（或手动输入）
2. **通知是图片时**：点「🖼️ 识别图片通知」→ 从相册选截图/照片 → 本地 OCR 把文字提取到输入框（首次使用需联网下载约20MB中文识别模型，之后完全离线）
3. 点「解析事件」→ 调用大模型 API 提取日程要素，自动过滤表情、问候、闲聊
4. 跳到核对页：标题 / 时间（日历+时间选择器）/ 地点 / 备注 / 提前提醒时长，全部可手动修改
5. 点「写入系统日历并提醒」→ 直接写入系统日历（CalendarProvider），按所选时长提前弹提醒
6. 「历史」页保存全部记录：点击可再次编辑，可重复写入日历，可删除
7. **日程更新**：当通知包含"延期/改期/调整"等词时，自动匹配历史中同名日程，展示"原时间→新时间"差异，确认后删除旧日程并写入新时间（原标题保持不变）

**时间识别说明**：解析时会把"手机当前日期、星期、时间"一并发给大模型，因此「明天、后天、下周三、本周五、9月10日、晚7点、7点半」等相对/口语时间都会换算成绝对公历日期和24小时制（如2026年8月29日收到"下周三晚7点"，会解析为 2026-09-02 19:00）。只提到时段没给钟点时（如"9月11日晚"），按 晚上→19:00、下午→14:00、中午→12:00、上午/早上→09:00 解析，不再误判为全天。通知里没有地点时，地点字段留空；完全没有任何时间信息时，才按"全天"事件写入。

**本地兜底**：没填API Key或API调用失败时，自动改用内置的本地规则解析，结果会标注"本地解析"，仍可手动校正后写入日历。本地解析支持：
- 阿拉伯数字和中文数字日期（`9月10日` / `九月十日` / `九月二十一号`）
- 阿拉伯数字和中文数字时间（`8:30` / `八点半` / `下午三点一刻` / `晚上九点四十五`）
- 相对日期（明天/后天/下周三/本周五）和时段词（早上/下午/晚上）
- 常见地点识别（教室/会议室/医院/公园/商场/车站/机场等）
- 标题自动清理（去掉日期时间连接词，只保留核心事件名）

---

## 二、编译环境

| 项目 | 要求 |
|---|---|
| IDE | Android Studio（建议 Hedgehog 2023.1.1 或更新版本，新版均可） |
| JDK | Android Studio 自带（JBR 17/21），无需单独安装 |
| Gradle | 8.7（Wrapper 已配好，首次构建自动下载，国内走腾讯镜像） |
| AGP / Kotlin | 8.5.2 / 1.9.24（项目内已锁定） |
| Android SDK | compileSdk 34（Android Studio 会自动下载缺失组件） |
| 运行设备 | Android 8.0（API 26）及以上 |

项目已配置阿里云Maven镜像+腾讯Gradle镜像，国内网络可直接同步。

> **关于安装包体积**：图片识别用的 ML Kit OCR 自带 native 库（每个 CPU 架构 10MB 上下），
> 因此 `app/build.gradle` 里用 `ndk { abiFilters 'arm64-v8a', 'armeabi-v7a' }` 只打包手机用的架构，
> 把安装包从 48MB 压到约 26MB。**如果你要用 Android Studio 的 x86_64 模拟器调试，
> 需临时把 `'x86_64'` 加进 `abiFilters`**，否则模拟器上会因缺少对应架构的 .so 而崩溃。

> 建议：若你的用户名/路径含中文导致个别电脑构建报编码错误，把整个 `NoticeCalendar` 文件夹移到纯英文路径（如 `D:\projects\NoticeCalendar`）再打开。

### 编译步骤

1. 打开 Android Studio → `Open` → 选择本项目文件夹 `NoticeCalendar`（注意选包含 `settings.gradle` 的这一层）
2. 等待右下角 Gradle Sync 完成（首次需联网下载依赖，几分钟）
3. 编译运行二选一：
   - **直接真机运行**：手机开USB调试连电脑，点工具栏 ▶ Run，自动安装打开
   - **只打包APK**：菜单 `Build → Build App Bundle(s) / APK(s) → Build APK(s)`，完成后右下角通知里点 `locate`，APK 在
     `app/build/outputs/apk/debug/app-debug.apk`

命令行方式（可选）：装好 JDK 17 后，在项目目录执行 `gradlew.bat assembleDebug`（Windows）即可。

---

## 三、如何填入自己的 API 密钥

**密钥不写进代码**。安装APP后，首页右上角点 **「设置」**，填三项：

1. **接口地址 Base URL**（OpenAI 兼容接口，不带 `/chat/completions` 后缀）
2. **API Key**（各服务商官网申请，仅保存在手机本机 SharedPreferences）
3. **模型名称**

填完点「保存」；建议先点「**测试连接**」验证可用。

常用服务商对照表：

| 服务商 | Base URL | 模型名示例 | 说明 |
|---|---|---|---|
| DeepSeek（推荐） | `https://api.deepseek.com/v1` | `deepseek-chat` | 便宜、中文效果好 |
| 智谱 GLM | `https://open.bigmodel.cn/api/paas/v4` | `glm-4-flash` | 有免费模型，注册即用 |
| Kimi（月之暗面） | `https://api.moonshot.cn/v1` | `moonshot-v1-8k` | |
| 通义千问 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | `qwen-plus` / `qwen-turbo` | 用"兼容模式"地址 |
| OpenAI | `https://api.openai.com/v1` | `gpt-4o-mini` | 需自备网络环境 |
| 本地 Ollama | `http://127.0.0.1:11434/v1` | `qwen2.5:7b` 等 | 完全离线，见下方说明 |

### 用本地 Ollama（可选，彻底本地运行）

1. 电脑上安装并运行 Ollama，拉一个模型：`ollama pull qwen2.5:7b`
2. 手机用USB线连电脑，执行：`adb reverse tcp:11434 tcp:11434`
3. APP设置里填：Base URL `http://127.0.0.1:11434/v1`，Key 随便填一个字符（如 `x`），模型 `qwen2.5:7b`

（也可让 Ollama 监听局域网 `OLLAMA_HOST=0.0.0.0`，Base URL 填电脑的局域网IP。）

---

## 四、打包 APK 并安装到手机

### 生成 APK

- 调试版（自用足够）：`Build → Build APK(s)` → `app/build/outputs/apk/debug/app-debug.apk`
- 正式版（可选）：`Build → Generate Signed App Bundle / APK → APK` → 第一次点 `Create new...` 建一个 keystore（记住密码）→ 选 release → 得到 `app-release.apk`

### 安装到手机

任选其一：

- **传文件安装**：把 APK 通过微信文件传输助手/QQ/数据线/网盘传到手机 → 文件管理器点击 APK → 允许"安装未知应用" → 安装
- **adb 安装**：手机开USB调试后连电脑，执行 `adb install -r app-debug.apk`

### 手机端准备

- 写入日历时APP会弹窗申请**日历权限**，点允许即可；部分国产ROM（MIUI/澎湃、EMUI、ColorOS）还需到 `设置 → 应用管理 → 通知日程助手 → 权限管理` 里手动把"日历"设为允许
- 如提示"未找到可用的系统日历账户"：打开系统自带的「日历」App，确认里面至少有一个日历账户（本地账户即可）

---

## 五、权限说明（仅申请必要权限）

| 权限 | 用途 | 说明 |
|---|---|---|
| `READ_CALENDAR` / `WRITE_CALENDAR` | 写入事件、设置提醒、选择日历账户 | 首次写入时运行时申请 |
| `INTERNET` | 调用你配置的大模型 API；首次使用图片识别时下载 OCR 模型 | API方式解析必需 |
| 相册选图 | 「识别图片通知」选择截图/照片 | 走系统相册选择器（Android 13+ Photo Picker / 旧版文档选择器），**无需申请存储权限** |
| 剪贴板 | 「从剪贴板粘贴」按钮读取 | Android 10+ 前台应用读取剪贴板**无需声明权限**，故清单中不申请 |

通知原文只会发送到**你自己配置的**大模型接口；**图片识别在本机离线完成，图片不会上传**；历史记录、密钥全部保存在手机本地，无任何账号体系。

---

## 六、项目结构

```
NoticeCalendar/
├── app/src/main/
│   ├── AndroidManifest.xml
│   ├── assets/
│   │   ├── donate.html              # 打赏页面
│   │   ├── wechat.png               # 微信收款码
│   │   └── alipay.png               # 支付宝收款码
│   ├── java/com/noticecalendar/app/
│   │   ├── MainActivity.kt          # 首页：粘贴/图片识别 → 解析 + 底部打赏卡片
│   │   ├── EditorActivity.kt        # 核对/编辑 → 写入日历（含更新模式banner）
│   │   ├── HistoryActivity.kt       # 历史记录列表
│   │   ├── SettingsActivity.kt      # API 配置 + 主题皮肤切换 + 检查更新 + 测试连接
│   │   ├── DonateActivity.kt        # 打赏页（WebView加载本地HTML）
│   │   ├── ocr/OcrHelper.kt         # 图片转文字（ML Kit 中文OCR，本地离线 + 断行合并）
│   │   ├── theme/ThemeManager.kt    # 主题管理器（5套预设主题）
│   │   ├── update/UpdateChecker.kt  # 自动更新检查（GitHub Release API）
│   │   ├── data/EventRecord.kt      # 记录模型
│   │   ├── data/EventRepository.kt  # 本地JSON文件存储
│   │   ├── data/SettingsStore.kt    # API密钥本地存储
│   │   ├── llm/LlmClient.kt         # OpenAI兼容接口客户端 + 提取提示词（含update类型）
│   │   ├── parse/LocalFallbackParser.kt  # 无API时的本地规则兜底解析（中文数字+标题清理）
│   │   ├── calendar/CalendarHelper.kt    # CalendarProvider 写入事件+提醒+删除事件
│   │   └── util/TimeUtil.kt
│   └── res/…                        # 布局、图标、主题（5套主题色）
├── build.gradle / settings.gradle   # 已配置国内镜像
└── gradle/wrapper/                  # Gradle 8.7 Wrapper
```

---

## 七、常见问题

- **日历详情里"类型"显示成邮箱账户**：那表示日程写进了该邮箱的同步日历。APP 默认**优先写入手机本地日历**；如需更换，到「设置 → 写入的日历账户」里选择（首次会申请日历权限）。
- **全天日程的"提前30分钟"提醒显示为前一天 23:30**：系统对全天事件按当天 0:00 倒推提醒时间，属正常现象；全天日程建议把提醒改为"提前1天"。
- **AI解析失败**：先用设置页「测试连接」排查：地址是否带了 `/v1`、Key 是否有效、账户是否有余额、模型名是否写对；访问 OpenAI 等海外接口需自备网络。
- **模型返回"不是JSON"**：个别模型不守规矩，换 `deepseek-chat` / `glm-4-flash` 等指令跟随好的模型即可（APP已做容错截取，偶发可重试）。
- **相对时间算错**：换算基准是**手机的当前日期时间**，确认手机时间准确即可。
- **写入日历失败**：见"手机端准备"第2、3条（权限、日历账户）。
- **卸载后记录还在吗**：不在。所有数据在应用私有目录，卸载即清除。

---

## 八、更新日志

### v1.5.3（2026-09-15）

- **修复"测试连接失败、报 HTML/429"**：根因是把服务商官网/控制台当作接口地址（`https://platform.deepseek.com/` 返回的是网页，HTTP 200 + text/html）。新增 `SettingsStore.normalizeBaseUrl()`：
  - `platform.deepseek.com` / `www.deepseek.com` / `deepseek.com` → `api.deepseek.com/v1`
  - 漏 `/v1` 自动补；多写 `/chat/completions` 自动去；无协议自动补 `https://`；结尾 `/` 去掉
  - 设置页回显修正后的值并提示保存
- **模型名自动纠正**（`normalizeModel()`）：`deepseek`→`deepseek-chat`、`kimi`→`moonshot-v1-8k`、`qwen`→`qwen-plus`、`glm`→`glm-4-flash`
- **错误信息可读化**：HTML 响应 → "接口地址返回的是网页而不是接口数据，请改成 API 地址（示例）"；401/402/403/404/429/5xx 均有中文说明；不再显示 HTML 源码片段
- 验证：21 个地址/模型用例外加"真实 OkHttp 请求本地假服务器返回 HTML"的 4 项断言，全部通过

### v1.5.2（2026-09-12）

- **修复志愿类通知提取不出标题**：事件词表补充 `志愿者|志愿服务|志愿|义工|公益|服务`。此前这类通知在本地解析下会退回"取原文第一行"，标题变成 `各位同学 的乘车安排有 变化所有的跟车志愿者(王鑫 陈仕鹏…` 这样的残句
- **新增"志愿 + 交通"组合标题**：通知里同时含志愿活动与 `乘车/坐车/大巴/包车/车辆/车队/接送/上车/发车` 时，标题合成为 **`乘车做志愿`**（不含交通词时仍走普通提取，如 `志愿者招募`）
- **修复 `志愿者面试` 被截成 `志愿者`**：新增可合并后缀词表（面试/招募/招新/服务/活动/培训/考试/笔试…），关键词后紧跟这些词时合并取出
- 回归验证：本次改动后重跑 12 个用例（含前两版修复的场景）**12/12 通过**

### v1.5.1（2026-09-12）

- **修复本地解析标题提取错误**（重要）：标题提取逻辑从"取原文第一行 + 正则挖掉日期"改为确定性的关键词定位算法——先找事件类型词（考试/大会/面试…），向前扫到最近的标点或引导动词，再剥掉词头噪音与"通知"类尾词
  - 「笔试（CET）于2026年12月12日举行，开考科目为英语四级和六级」→ `英语四级和六级笔试`（原来会变成 `笔试CET于 举行开考科目为英语四级和六级具`）
  - 「班会通知：明天下午3点在二教302开班会」→ `班会`（原来会带"通知"）
  - 「全国计算机二级考试报名开始」→ `计算机二级考试`
  - 「本周五下午2点实验室例会，请各组成员参加」→ `实验室例会`（原来会带"请各组成员参加"）
- **修复中文标点被当空格删除**导致的字符粘连（"…六级，具体…"曾被粘成"级具"）
- 修复后的规则以 9 个用例离线验证（含 v1.4 已修复场景的回归用例）全部通过
- 仅影响不填 API Key 时的本地解析；配置了大模型 API 的路径不变

### v1.5（2026-09-12）

- **图片通知识别（OCR）**：新增「🖼️ 识别图片通知（截图/照片）」按钮，从相册选图后自动提取文字填入输入框，核对后照常解析。解决"有些通知本身就是一张图片"的问题
- **本地离线识别**：采用 ML Kit 中文文字识别（中英文混排），在手机本地推理，**图片不上传服务器**；不填 API Key 也可用；首次使用联网下载约 20MB 模型
- **断行合并**：OCR 把一句通知拆成多行时自动接回，减少"标题/地点被截断"导致的解析错误
- **无需新增权限**：Android 13+ 走系统 Photo Picker，旧系统走 ACTION_OPEN_DOCUMENT，均不需要存储权限
- 首次识别模型下载失败时会提示"请连网后重试"，不影响粘贴文本等原有流程

### v1.4（2026-09-08）

- **修复深色模式文字看不清**：所有输入框显式设置主题感知的文字/占位符颜色，首页打赏卡片、设置页等写死的深色文字改为随亮/暗色切换
- **本地解析增强**：优先从通知中提取核心事件名（如"志愿者面试""党员大会"）；识别"面试地点在10110""候场地点在10113B"等房间号地点并完整保留（不再丢末尾字母）；标题截断从20字放宽到30字
- **无时间通知提示**：通知里没有日期和时间时，不再默默按"今天全天"预填，编辑页橙色高亮时间并提示用户确认补时间
- **版本号动态显示**：设置页版本号跟随构建配置自动显示，无需手动改布局文案

### v1.3（2026-09-01）

- **自动更新检查**：App启动时静默检查GitHub最新Release，有新版本自动弹出提示，显示当前版本、最新版本和更新日志
- **手动检查更新**：设置页新增「检查更新 / 获取最新版本」按钮，手动触发检查，无更新时提示"已是最新版本"
- **设置页关于区域**：新增版本号、开源协议信息
- **优化**：网络异常时自动检查静默失败，不打扰用户；手动检查失败时提示可手动访问GitHub查看

### v1.2（2026-09-01）

- **日程自动更新**：识别"延期/改期/调整"类通知，自动匹配历史日程，保留原标题，删除旧日程后写入新时间
- **中文数字解析**：本地解析器支持"九月十日""八点半""下午三点一刻"等中文数字日期和时间
- **标题自动清理**：自动去掉日期时间连接词，只保留核心事件名，日历上更醒目
- **5套主题换肤**：紫/蓝/绿/橙/深色，设置页切换即时生效
- **打赏支持**：首页底部"请作者喝瓶水"卡片，微信/支付宝收款码
- **扩展地点识别**：支持医院、公园、商场、车站、机场等常见生活地点
- **修复**：更新类型时间来源截断，避免取到联系截止时间等干扰；匹配时验证新旧时间不同，防止匹配到错误记录

### v1.1（2026-08-29）

- 初始版本：群通知AI解析 + 写入系统日历 + 历史记录
- 支持OpenAI兼容接口（DeepSeek/智谱GLM/Kimi/通义千问/本地Ollama等）
- 本地规则兜底解析（无API Key时可用）
- 相对时间自动换算（明天/后天/下周三等）
- 日历账户选择、提前提醒、权限管理
